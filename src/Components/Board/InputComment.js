...
function InputComment({boardItem,fetchData}) {
  ...
  return (
    <form action="" className='input-comment'>
      ...
      <input type="text" className='comment-writer' placeholder='닉네임' maxLength={20} value={commentWriter} onChange={(e)=>{
        setCommentWriter(e.target.value)
        
      }}/>
      <input type="password" className='comment-password' placeholder='비밀번호' maxLength={10} value={commentPassword} onChange={(e)=>{
        setCommentPassword(e.target.value)
        
      }}/>
      </div>
      <textarea className='comment-detail' value={commentDetail} placeholder='댓글을 입력하세요.' maxLength={200} onChange={(e)=>{
